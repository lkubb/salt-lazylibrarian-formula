...
We will see a quick setup to configure the ``lazylibrarian`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``lazylibrarian`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``lazylibrarian`` formulas under ``/srv/salt/lazylibrarian/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/lazylibrarian/parameters/dns:domain/
    ln -s dns:domain /srv/salt/lazylibrarian/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/lazylibrarian/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/lazylibrarian/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``lazylibrarian`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``lazylibrarian`` formulas under ``/srv/salt/lazylibrarian/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/lazylibrarian/parameters/roles
...
We create a configuration for the role ``lazylibrarian/server`` in ``/srv/salt/lazylibrarian/parameters/roles/lazylibrarian/server.yaml``:
...
We create another configuration for the role ``lazylibrarian/client`` in ``/srv/salt/lazylibrarian/parameters/roles/lazylibrarian/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles lazylibrarian/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles lazylibrarian/client
...
.. code-block:: console
    ...
    mv /srv/salt/lazylibrarian/parameters/dns:domain/  '/srv/salt/lazylibrarian/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-lazylibrarian-as-json:
      test.nop:
        - name: {{ lazylibrarian | json }}
...
Here is an example based on `template-formula/lazylibrarian/config/file.sls`_:
    ...
    lazylibrarian-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='lazylibrarian-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ lazylibrarian.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``lazylibrarian`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ lazylibrarian['winner'] }}
...
.. _template-formula/lazylibrarian/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/lazylibrarian/config/file.sls
